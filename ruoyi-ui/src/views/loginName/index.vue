  ...
  <div class="container">
    ...
    <div class="suspension">
      ...
      <div v-if="btn">
        ...
        <div v-show="!dropDown" class="flex_align drop_down" @click="dropDown = !dropDown">
          <p>点击展开</p>
          <i class="el-icon-caret-bottom"></i>
        </div>
        <!-- <query-card :card='cardContent'/> -->
        <!-- <card-img/> -->
        <!-- <button @click="addLine">添加线</button>
      <button @click="addStyle">添加颜色</button>
      <button @click="setLine">编辑线</button>
      <button @click="outLine">编辑结束</button> -->
      </div>
      <component
        v-else
        :is="currentComponent"
...
export default {
  ...
  data() {
    return {
      dataList:[],
      color:['rgb(74, 131, 242)','rgb(61, 216, 153)','rgb(247, 166, 98)','rgb(254, 112, 99)'],
      queryPlaceVal:true,
      setbtn: false,
      dropDown: true,
  ...
  watch: {},
  created() {
    Axios.get('http://api.tianditu.gov.cn/v2/administrative?keyword=156420302&childLevel=0&extensions=true&tk=525ecf8803a6268acc612ba1ae3e3065	').then((res)=>{
      console.log(res)
    })
  },
  mounted() {
  ...
  methods: {
    handleAlert(val){
      this.dataList = {
        ...
        jiedao:'永乐街道',
        laiyuan:'实地调查',
        jianjie:'武当山风景区1号停车场位于永乐路14号武当山风景区，靠近武当路、G316和金街，是游客游览武当山的重要交通枢纽。该停车场地理位置优越，与售票大厅相邻，方便游客购票后直接进入停车场。停车场设有明显的指示标志，方便游客快速找到停车位置。收费标准合理，每小时3元，24小时以内25元封顶。此外，停车场周边有多个公交站，包括武当山、武当山门等，可换乘206路、十堰武当山机场巴士2号线、十堰客运巴士班线等多条公交线路，为游客提供了便捷的交通选择'
      }
      this.message = {
        value:val
      }
  ...
  getImageSrc(index) {
    ...
    getBtn(value) {
      if(!!value&&value!=''){
     Axios.get(`http://api.tianditu.gov.cn/v2/search?postStr={"queryType":13,"start":0,"count":5,"specify":"156110000","dataTypes":"${value}"}&type=query&tk=525ecf8803a6268acc612ba1ae3e3065`).then((res)=>{
        console.log(res)
      }).catch((err)=>{
        console.log('er',err)
      })
    ...
    addLine() {
      ...
      map.centerAndZoom(new T.LngLat(116.40969, 39.9494), zoom);
      var points = [];
      points.push(new T.LngLat(116.41136, 39.97569));
      points.push(new T.LngLat(116.411794, 39.9068));
      points.push(new T.LngLat(116.32969, 39.9294));
    ...
    LineClick(e) {
      ...
      console.log("您点击了线",e.target.ht.join(','), 'http://api.tianditu.gov.cn/geocoder?postStr='+{...e.target.ht}+'&type=geocode&tk=525ecf8803a6268acc612ba1ae3e3065');
         this.currentComponent = 'searchLoad';
      this.queryPlaceVal = true
      this.btn = false;
    },
